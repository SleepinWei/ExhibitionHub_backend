...
public interface SubMapper extends BaseMapper<Subscription> {
    @Select("SELECT DISTINCT subscription.ex_id,visitdate,name,venue_name,organizer,begin_date,end_date,province,city,area,address,link,tag_id " +
            "FROM subscription,exhibition,exhibition_tag " +
            "WHERE user_id=#{userid} and subscription.ex_id=exhibition.id and exhibition.id=exhibition_tag.ex_id and visitdate>=#{begin} and visitdate<=#{end}")
